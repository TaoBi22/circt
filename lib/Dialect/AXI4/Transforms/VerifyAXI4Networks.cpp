...
/// Best-effort name for the module a manager_port's node instantiates, for
/// diagnostics. Falls back to a generic label if the node isn't produced
/// directly by an 'axi4.node' (e.g. a block argument) -- this is
/// diagnostic-only and never affects correctness of the analysis.
static std::string describeManager(ManagerPortOp mgr) {
  if (auto node = mgr.getNode().getDefiningOp<NodeOp>())
    return ("@" + node.getModule()).str();
...
static bool isForwarder(Operation *op) {
  return op && !isa<ManagerPortOp>(op) &&
            ...
            for (TrackedWindow m : merged)
              if (overlaps(n.base, n.size, m.base, m.size))
                return xbar.emitOpError()
                       ...
                       << ") is reachable from both " << describeManager(n.source)
                       << " and " << describeManager(m.source);
            merged.push_back(n);
          }
...
struct VerifyAXI4NetworksPass
  ...
  void runOnOperation() override {
    ...
    // first.
    DenseMap<Value, SmallVector<SubordinatePortOp>> downstreamSubs;
    auto computeDownstream = [&](Value port) {
      SmallPtrSet<Operation *, 4> seen;
      SmallVector<SubordinatePortOp> subs;
      for (Operation *user : port.getUsers()) {
        if (auto sub = dyn_cast<SubordinatePortOp>(user)) {
          if (seen.insert(sub).second)
            subs.push_back(sub);
        } else if (isForwarder(user)) {
          for (Value out : portResults(user))
            for (SubordinatePortOp s : downstreamSubs.lookup(out))
    ...
    module.walk([&](ManagerPortOp mgr) { computeDownstream(mgr.getPort()); });
    ...
    module.walk([&](SubordinatePortOp sub) {
          ...
          for (BurstSpecAttr want : r.bursts)
            if (!supports(w.getBurstSpecs(), want)) {
              sub.emitOpError()
                  ...
                  << describeManager(r.source) << " to addresses [" << r.base
                  << ", " << r.base + r.size << ")";
              anyError = true;
            }
        }
    ...
    module.walk([&](ManagerPortOp mgr) {
      ...
      struct ConeWin {
        uint64_t base, size;
        SubordinatePortOp op;
      };
      SmallVector<ConeWin> coneWins;
      for (SubordinatePortOp s : downstreamSubs.lookup(mgr.getPort()))
