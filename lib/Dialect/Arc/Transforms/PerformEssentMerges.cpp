...
bool ArcEssentMerger::canMergeArcs(CallOpInterface firstArc,
  ...
  // // WIP: can we speed up our big check with a dominance info?
  // for (auto secondRes : secondArc->getResults()) {
  //   if (dom.dominates(secondRes, firstArc.getOperation())) {
  //     // If our second arc's result dominates the first arc, it must use it
  //     and
  //     // we'll create a comb cycle if we merge them
  //     return false;
  //   }
  // }
