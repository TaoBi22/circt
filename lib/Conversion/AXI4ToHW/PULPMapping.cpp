...
static std::string pulpXbarSource(StringRef name, XbarOp xbar) {
  ...
  unsigned ruleWidth = addrWidth <= 32 ? 32 : 64;
  ...
  uint64_t mask = ruleWidth == 64 ? ~uint64_t{0} : (uint64_t{1} << 32) - 1;
  ...
  for (auto [index, value] : llvm::enumerate(xbar.getDownstream())) {
    for (WindowAttr window :
      ...
      rules.push_back(("    '{idx: " + Twine(index) +
                       ", start_addr: " + Twine(ruleWidth) + "'h" +
                       llvm::utohexstr(window.getBase()) +
                       ", end_addr: " + Twine(ruleWidth) + "'h" +
  ...
  os << "    default:            '0\n";
  os << "  };\n";
  os << "  localparam axi_pkg::xbar_rule_" << ruleWidth << "_t ["
     << rules.size() << "-1:0] AddrMap = '{\n";
  os << llvm::join(rules, ",\n") << "\n";
  os << "  };\n";
  ...
  os << "    .rule_t        (axi_pkg::xbar_rule_" << ruleWidth << "_t)\n";
  os << "  ) i_xbar (\n";
  os << "    .clk_i                 (clk_i),\n";
  os << "    .rst_ni                (rst_ni),\n";
  os << "    .test_i                (1'b0),\n";
