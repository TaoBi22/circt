...
/// Produce the SystemVerilog wrapper for one cut configuration. Simpler than the
/// crossbar: one symmetric upstream/downstream face and no address map, so it
/// just spill-registers via PULP's axi_cut. Unlike the xbar, the write and read
/// ID widths are kept independent (axi_cut never inspects them).
