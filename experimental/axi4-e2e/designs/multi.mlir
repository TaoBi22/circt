...
!sub_a = !axi4.port<addr_width = 32, data_width = 64, write_id_width = 5, read_id_width = 5, user_width = 1, windows = <<base = 0x0, last = 0xfff, burst_specs = <<incr, len = 2>, <incr, len = 4>>>>, outstanding_writes = 8, outstanding_reads = 8>
!sub_b = !axi4.port<addr_width = 32, data_width = 64, write_id_width = 5, read_id_width = 5, user_width = 1, windows = <<base = 0x1000, last = 0x1fff, burst_specs = <<incr, len = 2>, <incr, len = 4>>>>, outstanding_writes = 8, outstanding_reads = 8>
...
!aw5 = !hw.struct<id: i5, addr: i32, len: i8, size: i3, burst: i2, lock: i1, cache: i4, prot: i3, qos: i4, region: i4, atop: i6, user: i1>
!b5 = !hw.struct<id: i5, resp: i2, user: i1>
!ar5 = !hw.struct<id: i5, addr: i32, len: i8, size: i3, burst: i2, lock: i1, cache: i4, prot: i3, qos: i4, region: i4, user: i1>
!r5 = !hw.struct<id: i5, data: i64, resp: i2, last: i1, user: i1>

hw.module @mgr_module_a(in %clk_i : !seq.clock, in %rst_ni : i1, out axi : !mgr,
  ...
  %aw = hw.struct_create (%c4, %awaddr_w_a, %awlen2, %arsize, %arburst_incr, %f,
                          %c4, %c3, %c4, %c4, %atop0, %f) : !aw
  %w = hw.struct_create (%wdata_sel, %wstrb_full, %w_beat_idx_q, %f) : !w
  %ar = hw.struct_create (%c4, %araddr_a, %arlen4, %arsize, %arburst_incr, %f,
                          %c4, %c3, %c4, %c4, %f) : !ar
  %port, %awready, %wready, %b, %bvalid, %arready, %r, %rvalid =
    axi4.channel_structs_to_port %clk_i, %rst_ni
      aw %aw, %in_p2 w %w, %in_p3 b %in_p4 ar %ar, %arvalid r %t : !mgr
...
hw.module @mgr_module_b(in %clk_i : !seq.clock, in %rst_ni : i1, out axi : !mgr,
                        out done : i1, out beat0 : i64, out beat1 : i64,
                        out beat2 : i64, out beat3 : i64, out after_beat0 : i64,
                        out after_beat1 : i64, out after_beat2 : i64,
                        out after_beat3 : i64) {
  ...
  %aw = hw.struct_create (%c4, %awaddr_w_b, %awlen2, %arsize, %arburst_incr, %f,
                          %c4, %c3, %c4, %c4, %atop0, %f) : !aw
  %w = hw.struct_create (%wdata_sel, %wstrb_full, %w_beat_idx_q, %f) : !w
  %ar = hw.struct_create (%c4, %araddr_b, %arlen4, %arsize, %arburst_incr, %f,
                          %c4, %c3, %c4, %c4, %f) : !ar
  %port, %awready, %wready, %b, %bvalid, %arready, %r, %rvalid =
    axi4.channel_structs_to_port %clk_i, %rst_ni
      aw %aw, %in_p2 w %w, %in_p3 b %in_p4 ar %ar, %arvalid r %t : !mgr
...
hw.module @sub_module5_a(in %clk_i : !seq.clock, in %rst_ni : i1, in %axi : !sub_a) {
  ...
  %b = hw.struct_create (%bid_q, %c0_i2, %false) : !b5
  %r = hw.struct_create (%rid_q, %selected, %c0_i2, %is_last_beat, %false) : !r5
  ...
  %awid = hw.struct_extract %aw["id"] : !aw5
  %awaddr = hw.struct_extract %aw["addr"] : !aw5
  %wdata = hw.struct_extract %w["data"] : !w
  %wlast = hw.struct_extract %w["last"] : !w
  %arid = hw.struct_extract %ar["id"] : !ar5
  %araddr = hw.struct_extract %ar["addr"] : !ar5
  %arlen = hw.struct_extract %ar["len"] : !ar5
  ...
  %rid_next = comb.mux %accept, %arid, %rid_q : i5
  %rid_q = seq.compreg %rid_next, %clk_i reset %rst, %c0_i5 : i5
  ...
  %bid_next = comb.mux %aw_accept, %awid, %bid_q : i5
  %bid_q = seq.compreg %bid_next, %clk_i reset %rst, %c0_i5 : i5
...
hw.module @sub_module5_b(in %clk_i : !seq.clock, in %rst_ni : i1, in %axi : !sub_b) {
  %false = hw.constant false
  %true = hw.constant true
  %c0_i2 = hw.constant 0 : i2
  %c1_i2 = hw.constant 1 : i2
  ...
  // Distinct starting contents from sub_module5_a's, so the waveform/
  ...
  // ---- Read side: 2-state FSM (see sub_module5_a's comment).
  %state_q = seq.compreg %state_next, %clk_i reset %rst, %false : i1
  %idle = comb.xor %state_q, %true : i1
  %accept = comb.and %arvalid, %idle : i1
  %r_handshake = comb.and %state_q, %rready : i1
  ...
  // The window's base is stripped by the xbar, so bits 4:3 index this RAM.
  %idx_start = comb.extract %araddr from 3 : (i32) -> i2
  %idx_inc = comb.add %idx_q, %c1_i2 : i2
  %idx_after_beat = comb.mux %r_handshake, %idx_inc, %idx_q : i2
  %idx_next = comb.mux %accept, %idx_start, %idx_after_beat : i2
...
hw.module @AXITop(in %clk_i : !seq.clock, in %rst_ni : i1) {
  %mgr_a, %done_a, %a_beat0, %a_beat1, %a_beat2, %a_beat3,
  %a_after0, %a_after1, %a_after2, %a_after3 =
    hw.instance "mgr_a" @mgr_module_a(clk_i: %clk_i: !seq.clock, rst_ni: %rst_ni: i1)
      -> (axi: !mgr, done: i1, beat0: i64, beat1: i64, beat2: i64, beat3: i64,
          after_beat0: i64, after_beat1: i64, after_beat2: i64, after_beat3: i64)
  %mgr_b, %done_b, %b_beat0, %b_beat1, %b_beat2, %b_beat3,
  %b_after0, %b_after1, %b_after2, %b_after3 =
    hw.instance "mgr_b" @mgr_module_b(clk_i: %clk_i: !seq.clock, rst_ni: %rst_ni: i1)
      -> (axi: !mgr, done: i1, beat0: i64, beat1: i64, beat2: i64, beat3: i64,
          after_beat0: i64, after_beat1: i64, after_beat2: i64, after_beat3: i64)
  %sub_a, %sub_b = axi4.xbar %clk_i, %rst_ni mgrs %mgr_a, %mgr_b
    : (!mgr, !mgr) -> (!sub_a, !sub_b)
  hw.instance "sub_a" @sub_module5_a(clk_i: %clk_i: !seq.clock, rst_ni: %rst_ni: i1,
                                    axi: %sub_a: !sub_a) -> ()
  hw.instance "sub_b" @sub_module5_b(clk_i: %clk_i: !seq.clock, rst_ni: %rst_ni: i1,
                                     axi: %sub_b: !sub_b) -> ()
  hw.output
}
