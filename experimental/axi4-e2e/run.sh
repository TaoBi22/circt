...
#   1. lower       --lower-axi4-to-hw=pulp-mapping=true
#   2. emit        --lower-seq-to-sv --canonicalize --export-verilog -> the full
#                  design (top module + xbar wrappers).
...
for design in "$here"/designs/*.mlir; do
  ...
  if ! "$CIRCT_OPT" "$design" --lower-axi4-to-hw=pulp-mapping=true -o "$low" 2>"$build/$name.lower.log"; then
  ...
  # modules are its manager/subordinate endpoints.
  top=AXITop
  ...
  # Nothing from the AXI4 dialect may survive: the endpoints' channel-struct
  # bridges cancel against the ones the pass materializes at each boundary.
  grep -qE "axi4\." "$low" && { ok=0; fail "structural $name: abstract axi4 ops survived lowering"; }
