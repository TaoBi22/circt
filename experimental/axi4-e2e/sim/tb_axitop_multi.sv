...
module tb_axitop_multi;
  ...
  logic rst_ni = 0;  // active-low, asserted at time 0

  AXITop dut (.clk_i(clk_i), .rst_ni(rst_ni));
  ...
  initial begin
    ...
    // Release reset after a few cycles; the real PULP axi_xbar resets,
    // then the read-write-read traffic runs against it.
    repeat (4) @(posedge clk_i);
    rst_ni = 1;

    for (int i = 0; i < CYCLE_BOUND; i++) begin
      @(posedge clk_i);
      if (!seen_a && dut.mgr_a.done) begin
        seen_a = 1;
        done_cycle_a = i;
      end
      if (!seen_b && dut.mgr_b.done) begin
        seen_b = 1;
        done_cycle_b = i;
      end
    ...
    if (!seen_a || !seen_b) begin
      ...
      $fatal;
    end else if (dut.mgr_a.beat0 !== EXPECTED_A_BEAT0 ||
                 dut.mgr_a.beat1 !== EXPECTED_A_BEAT1 ||
                 dut.mgr_a.beat2 !== EXPECTED_A_BEAT2 ||
                 dut.mgr_a.beat3 !== EXPECTED_A_BEAT3 ||
                 dut.mgr_b.beat0 !== EXPECTED_B_BEAT0 ||
                 dut.mgr_b.beat1 !== EXPECTED_B_BEAT1 ||
                 dut.mgr_b.beat2 !== EXPECTED_B_BEAT2 ||
                 dut.mgr_b.beat3 !== EXPECTED_B_BEAT3) begin
      $display("FAIL: first read burst mismatch");
      $display("  mgr_a: %0h,%0h,%0h,%0h",
                dut.mgr_a.beat0, dut.mgr_a.beat1,
                dut.mgr_a.beat2, dut.mgr_a.beat3);
      $display("  mgr_b: %0h,%0h,%0h,%0h",
                dut.mgr_b.beat0, dut.mgr_b.beat1,
                dut.mgr_b.beat2, dut.mgr_b.beat3);
      $fatal;
    end else if (dut.mgr_a.after_beat0 !== EXPECTED_A_BEAT0 ||
                 dut.mgr_a.after_beat1 !== NEW_A_WORD1 ||
                 dut.mgr_a.after_beat2 !== NEW_A_WORD2 ||
                 dut.mgr_a.after_beat3 !== EXPECTED_A_BEAT3 ||
                 dut.mgr_b.after_beat0 !== EXPECTED_B_BEAT0 ||
                 dut.mgr_b.after_beat1 !== NEW_B_WORD1 ||
                 dut.mgr_b.after_beat2 !== NEW_B_WORD2 ||
                 dut.mgr_b.after_beat3 !== EXPECTED_B_BEAT3) begin
      $display("FAIL: read-after-write mismatch");
      $display("  mgr_a after: %0h,%0h,%0h,%0h",
                dut.mgr_a.after_beat0, dut.mgr_a.after_beat1,
                dut.mgr_a.after_beat2, dut.mgr_a.after_beat3);
      $display("  mgr_b after: %0h,%0h,%0h,%0h",
                dut.mgr_b.after_beat0, dut.mgr_b.after_beat1,
                dut.mgr_b.after_beat2, dut.mgr_b.after_beat3);
      $fatal;
    end else begin
