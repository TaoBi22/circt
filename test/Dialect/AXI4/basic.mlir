...
hw.module @Crossbar(in %clk : !seq.clock, in %rst_ni : i1) {
  ...
  axi4.abstract_subordinate %clk, %rst_ni, %sub1 : !sub1
  // CHECK: axi4.abstract_subordinate %clk, %rst_ni, %[[XBAR]]#1 :
  axi4.abstract_subordinate %clk, %rst_ni, %sub2 : !sub2
  // CHECK: axi4.abstract_subordinate %clk, %rst_ni, %[[XBAR]]#2 :
  axi4.abstract_subordinate %clk, %rst_ni, %sub3 : !sub3
}
...
hw.module @LongerBurstSubordinate(in %clk : !seq.clock, in %rst_ni : i1) {
  ...
  // CHECK: axi4.abstract_subordinate %clk, %rst_ni, %[[XBAR]] :
  axi4.abstract_subordinate %clk, %rst_ni, %sub : !long_sub
}
...
// Typedefs for a converter halving the data width, which doubles the beats a
// burst takes
...
// Typedef for a downstream port taking longer bursts than the conversion needs
...
hw.module @DummiesXbar(in %clk : !seq.clock, in %rst_ni : i1) {
  ...
  // CHECK: %[[MEM_ACCESS:.+]] = axi4.dummies.ext_subordinate %clk, %rst_ni, %[[XBAR]] window <base = 0x0, last = 0xfff, burst_specs = <<incr, len = 16>>>
  ...
  // CHECK: %[[PERIPH_ACCESS:.+]] = axi4.dummies.ext_subordinate %clk, %rst_ni, %[[XBAR]] window <base = 0x1000, last = 0x1fff, burst_specs = <<fixed, len = 4>>>
  ...
  // CHECK: axi4.dummies.accesses %[[CORE_ACCESS]] -> %[[MEM_ACCESS]] with <<incr, len = 16>> {a}
  axi4.dummies.accesses %core_access -> %mem_access with <<incr, len = 16>> {a}
  // CHECK: axi4.dummies.accesses %[[CORE_ACCESS]] -> %[[PERIPH_ACCESS]] with <<fixed, len = 4>>
