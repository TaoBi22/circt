...
// block, so ops in a different region (here, a second network nested in an
...
%xbar = axi4.xbar %clk mgrs %mgr : (!axi4.port<32, 64, 4>) -> !axi4.port<32, 64, 4>
axi4.subordinate_port %xbar, %clk node %sub_node0 {
  port_mapping = #axi4.port_wires<"clk", "s0">,
  access = [#axi4.window<base = 0, size = 4096, burst_specs = [<fixed>]>],
  outstanding_requests = 4 : ui32
} : !axi4.port<32, 64, 4>
axi4.subordinate_port %xbar, %clk node %sub_node1 {
  port_mapping = #axi4.port_wires<"clk", "s0">,
...
%xbar = axi4.xbar %clk mgrs %mgr : (!axi4.port<128, 64, 4>) -> !axi4.port<128, 64, 4>
axi4.subordinate_port %xbar, %clk node %sub_node {
  port_mapping = #axi4.port_wires<"clk", "s0">,
  access = [#axi4.window<base = 0, size = 4096, burst_specs = [<fixed>]>],
  outstanding_requests = 4 : ui32
