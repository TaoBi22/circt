...
// downstream subordinates, each side an array of packed req/resp structs. The AW
// channel carries a 6-bit `atop` field our canonical form lacks (tied to 0).

// CHECK-LABEL: hw.module.extern @axi_xbar_1u1d_a32_d64_i4_o4(
// CHECK-SAME: in %clk_i : i1
// CHECK-SAME: in %slv_ports_req_i : !hw.array<1xstruct<aw: !hw.struct<id: i4, addr: i32, len: i8, size: i3, burst: i2, lock: i1, cache: i4, prot: i3, qos: i4, region: i4, atop: i6>, aw_valid: i1, w: !hw.struct<data: i64, strb: i8, last: i1>, w_valid: i1, b_ready: i1, ar: !hw.struct<id: i4, addr: i32, len: i8, size: i3, burst: i2, lock: i1, cache: i4, prot: i3, qos: i4, region: i4>, ar_valid: i1, r_ready: i1>>
// CHECK-SAME: out slv_ports_resp_o : !hw.array<1xstruct<aw_ready: i1, ar_ready: i1, w_ready: i1, b_valid: i1, b: !hw.struct<id: i4, resp: i2>, r_valid: i1, r: !hw.struct<id: i4, data: i64, resp: i2, last: i1>>>
// CHECK-SAME: out mst_ports_req_o : !hw.array<1xstruct<aw: !hw.struct<{{.*}}, atop: i6>, aw_valid: i1, {{.*}}, r_ready: i1>>
// CHECK-SAME: in %mst_ports_resp_i : !hw.array<1xstruct<aw_ready: i1, {{.*}}, r: !hw.struct<id: i4, data: i64, resp: i2, last: i1>>>

// The upstream manager's request channels are packed into a req struct - AW
// first bridged to `aw_chan_t` with atop tied to 0 - and arrayed into the slave
// request port. The subordinate's responses are likewise packed into the master
// response port.
// CHECK-DAG: %[[ATOP:.+]] = hw.constant 0 : i6
// CHECK-DAG: %[[AWCHAN:.+]] = hw.struct_create ({{.*}}, %[[ATOP]]) : !hw.struct<id: i4, addr: i32, len: i8, size: i3, burst: i2, lock: i1, cache: i4, prot: i3, qos: i4, region: i4, atop: i6>
...
// CHECK-DAG: %{{.+}} = hw.struct_extract %[[MSTAW]]["id"] : !hw.struct<id: i4, {{.*}}, atop: i6>
...
// CHECK: %[[X1AWID:.+]] = hw.struct_extract %[[X1AW]]["id"] : !hw.struct<id: i4, {{.*}}, atop: i6>
