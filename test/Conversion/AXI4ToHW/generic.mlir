...
hw.module @AXITop(in %clk_i : i1, in %rst_ni : i1, in %sum_i : i8, out carry_o : i1) {
  %clk = builtin.unrealized_conversion_cast %clk_i : i1 to !axi4.clock
  %rst = builtin.unrealized_conversion_cast %rst_ni : i1 to !axi4.reset
  ...
  axi4.generic_input %sum_i, "sum_i" node %mnode : i8
  %carry = axi4.generic_output "carry_o" node %mnode : i1
  hw.output %carry : i1
}
